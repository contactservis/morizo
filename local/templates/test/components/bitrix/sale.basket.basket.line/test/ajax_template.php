<?
/**
 *  фото товара брать не из стандартных полей, 
 *  а из папки на сервере (/upload/photo/) (название файла соответствует артикулу)
 */
$rootDirImg = '/upload/photo/';
						...
						<div class="bx-basket-item-list-item-img">
							<?if ($arParams["SHOW_IMAGE"] == "Y" && $v["PICTURE_SRC"]):?>
								...
								<?else:?>
									<? 
										/**
										 * Определение типа изображения и существования файла
										 */
										if( exif_imagetype($rootDirImg.$v["CML2_ARTICLE"].".jpg") ){
                      ?>
                        <img src="<?=$rootDirImg.$v["CML2_ARTICLE"].".jpg"?>" alt="<?=$v["NAME"]?>" />
                      <?
										}elseif ( exif_imagetype($rootDirImg.$v["CML2_ARTICLE"].".png") ) {
											?>
                        <img src="<?=$rootDirImg.$v["CML2_ARTICLE"].".png"?>" alt="<?=$v["NAME"]?>" />
                      <?
										}else {
                      ?>
                        <img src="<?=$rootDirImg."noimage.jpg"?>" alt="<?=$v["NAME"]?>" />
                      <?
                    }
									?>
								<?endif?>
